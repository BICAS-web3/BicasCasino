...
const TokenPicker: FC<TokenPickerProps> = (props) => {
  var [currentNetwork, currentToken, pickToken, availableTokens] = useUnit([
  ...
  return (
    <div className={s.token_picker_container}>
      <div className={s.picked_token}></div>
      <div className={s.tokens_variants}></div>
    </div>
  );
};
    ...
    <div>
      <div className={s.wager_container}>
        ...
        <div className={s.wager_input_container}>
          <input
            className={s.wager_input}
            value={props.valueDollars}
            onChange={props.onChangeDollarsHandler}
          ></input>
          <div
            style={{
              display: "flex",
        ...
        <div className={s.wager_percentage_picker}>
          <div
            className={s.percentage_pick}
            onClick={() => props.onClickHandlers(0.25)}
          >
            25%
          </div>
          <div
            className={s.percentage_pick}
            onClick={() => props.onClickHandlers(0.5)}
          >
            50%
          </div>
          <div
            className={s.percentage_pick}
            onClick={() => props.onClickHandlers(0.75)}
          >
            75%
          </div>
          <div
            className={s.percentage_pick}
            onClick={() => props.onClickHandlers(1)}
          >
            100%
          </div>
        </div>
