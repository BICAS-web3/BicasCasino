...
const ConnectWalletButton: FC<ConnectWalletButtonProps> = (props) => {
	...
	return (
		<div className={s.connect_wallet_button_wrap}>
			<div className={s.connect_wallet_button} onClick={handleConnectWalletBtn} ref={buttonRef}>
				Connect Wallet
			</div>
			<div
			...
			>
				<AvaibleWallet
					hideAvaibleWallet={hideAvaibleWallet}
...
export const NetworkPicker: FC<NetworkPickerProps> = (props) => {
	...
	for (let availableNetwork of availableNetworks.networks) {
		if (
			currentNetwork != null &&
			availableNetwork.network_id == currentNetwork.network_id
		) {
			continue;
		}
		chains.push(
			<div
				...
				onClick={async () => {
					const rpcs = (await (
						await Api.getRpcsFx({ network_id: availableNetwork.network_id })
					).body) as Api.T_Rpcs;
					const networkParams = {
	...
	return (
		<div
			className={s.network_picker_container}
			...
			>
				{currentWalletAddress == null ? (
					...
					<>
						...
						<div className={s.network_picker_arrow}>{">"}</div>
					</>
				)}
			</div>
			<div className={s.networks_list}>{chains}</div>
		</div>
	);
};
...
export const Header: FC<HeaderProps> = (props) => {
	...
	useEffect(() => {
		...
		if (web3Provider != null) {
			...
			window.ethereum.on("chainChanged", (network_id: any) =>
	...
	const queryAllExplorers = async () => {
		const explorers = (await Api.getAllExplorers())
			.body as Api.T_BlockExplorers;
	...
	const checkMetamaskConnection = async () => {
		...
		//     console.error(err);
		// });
		if (web3Provider?.provider.request == null) {
			return;
		}
	...
	const networkChangeHandler = async (
		network_id: any,
		available_networks: Api.T_Networks | undefined
	) => {
		...
		if (
			currentNetwork != null &&
			network_id_number == currentNetwork.network_id
		) {
		...
		if (network != undefined) {
			...
			var available_tokens = (
				await Api.getTokens({ network_id: network.network_id })
	...
	const checkCurrentNetwork = async (available_networks: any) => {
		...
		await web3Provider?.provider
			...
			.then((network_id) =>
				networkChangeHandler(network_id, available_networks)
			)
			.catch((err) => {
				console.error(err);
			});
