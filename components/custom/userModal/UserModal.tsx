...
export const UserModal: FC<UserModalProps> = () => {
  ...
  const [isDrax, setDrax, info] = useUnit([
    UserModel.$isDrax,
    UserModel.setIsDrax,
    UserModel.$userInfo
  ])
  ...
  useEffect(() => {
    if (userInfo?.id) {
      ;(async () => {
        const data = await getUserBetData({ id: userInfo.id })
        if (data.status === 'OK') {
          setBetData((data as any).body)
        }
      })()
    }
  }, [userInfo])
            ...
            {/* <div className='rounded-[50px] flex gap-[5px] w-fit bg-[#121212] border border-[#212121] p-[4px]'>
              <div
                ...
                className={`flex cursor-pointer gap-[10px] ${
                  !isDrax && 'bg-[#202020]'
                } p-[3px] rounded-[10px] text-[13px] font-medium`}
              >
                <BonusIco />
                {t(`modals.profile.bonus`)}
              </div>
              <div
