    ...
    <>
      ...
      >
        {slides.map((item, ind) => (
          <SwiperSlide
            key={ind}
            className='vip_swiper_slide h-full flex flex-col gap-[2px]'
          >
            ...
            <div className='flex flex-col h-full'>
              {item.list.map((item2, ind2) => (
                <div
                  key={ind2}
                  data-bg={ind2 % 2 === 0}
                  className={`flex min-h-[70px] gap-[5px] h-full items-center justify-center ${
                    ind2 % 2 === 0 ? 'bg-[#1A1A1A]' : 'bg-[#181818]'
                  }`}
                >
                  {item2.ico && item2.ico}
                  {item2.title && (
                    <span className='text-[15px] font-semibold'>
                      {item2.title?.includes('Level')
                        ? `${t(`pages.vip.level`)} ${ind2 + 1}`
                        : item2.title}
                    </span>
                  )}
                </div>
